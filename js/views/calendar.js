...
export async function render(root) {
  ...
  const row = (a, { withDate = false } = {}) => {
    ...
    const sub = isOp(a)
      ...
      : [withDate ? fmtDayMonth(a.date) : null, fmtTime(a.date), lower(apptLabel(a)), pr ? procLabel(pr.type) : (kindLabel(a.kind) !== apptLabel(a) ? kindLabel(a.kind) : null), a.notes || null].filter(Boolean).join(' · ');
    return `
    ...
    return `
      <button class="upcoming-card" type="button" data-appt="${a.id}">
        <div>
          ...
          <div class="sub">${(isOp(a) ? [apptLabel(a), lower(t('op.row')), fmtTime(a.date)] : [pr ? procLabel(pr.type) : null, lower(apptLabel(a)), fmtTime(a.date)]).filter(Boolean).map(esc).join(' · ')}</div>
        </div>
        <div>
  ...
  function paintMonth() {
    ...
    const dot = (a) => `<i class="cal-dot ${isOp(a) ? 'op' : ''} ${a.status === 'done' ? 'done' : a.status === 'missed' ? 'warn' : (parseDate(a.date) < today && a.status === 'planned') ? 'warn' : ''}"></i>`;
