...
7. [ ] Karşılaştırma kaydırıcısı tutamaçtan tutunca takılıyor: zoom denetleyicisinin tek parmakta işaretçi yakalaması tutamacın olaylarını yutuyordu; yakalama yalnızca pinch ve yakınlaştırılmış kaydırmada.
8. [ ] Alt çubuk sayfa geçişlerinde pırpırlıyor: gizle-göster ile yeniden yerleştirme kaldırıldı, cam katmanlar kendi katmanına alındı.
9. [ ] Uygulama adı ve ikonu uygulama içinde: Hastalar başlığının üstünde 20px ikon + Curalis; kilit ekranında 56px ikon.
